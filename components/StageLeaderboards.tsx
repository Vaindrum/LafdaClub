...
export default function StageLeaderboards() {
  const [stages, setStages] = useState<Stage[]>([]);

  useEffect(() => {
    const fetchData = async () => {
      try {
        const { data } = await axiosInstance.get('stats/stageLeaderboards');
        setStages(data.topStages); // expecting an array sorted by rank
      } catch (err) {
        console.error('Failed to fetch stages leaderboard:', err);
      }
    };
    fetchData();
  }, []);
  ...
  const topThree = stages.slice(0, 3);
  const others = stages.slice(3);
        ...
        {others.map((s, idx) => {
          ...
          return (
            ...
            >
              ...
              <p className="font-semibold">{s.timesPicked}</p>
            </div>
          );
        })}
      </div>
